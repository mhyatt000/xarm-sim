...
def _make_nyx_light_fields(light_field_cfgs):
    light_fields = []
    for cfg in light_field_cfgs:
        if isinstance(cfg, nps.LightFieldAsset):
            light_fields.append(cfg)
            continue

        light_field = nps.LightFieldAsset()
        light_field.type = cfg.get("type", nps.ELightFieldType.GaussianField)
        light_field.uri = cfg["uri"]
        if "position" in cfg:
            light_field.position = nps.float3(*cfg["position"])
        if "rotation" in cfg:
            light_field.rotation = nps.quaternion(*cfg["rotation"])
        if "scale" in cfg:
            scale = cfg["scale"]
        ...
        if "tint" in cfg:
            light_field.tint = nps.float3(*cfg["tint"])
        light_fields.append(light_field)
    return tuple(light_fields)
...
class GraspEnv:
    def __init__(
        ...
        # == add ground ==
        floor_surface = gs.surfaces.BSDF(
            color=tuple(env_cfg.get("floor_color", (1.0, 1.0, 1.0))),
            metallic=env_cfg.get("floor_metallic", 0.0),
            roughness=env_cfg.get("floor_roughness", 0.25),
        )
        self.scene.add_entity(
            gs.morphs.Plane(visualization=env_cfg.get("floor_visualization", True), collision=True),
            surface=floor_surface,
            # gs.morphs.URDF(file="urdf/plane/plane.urdf", fixed=True),
        )

        for wall_cfg in env_cfg.get("walls", []):
            self.scene.add_entity(
                gs.morphs.Box(
                    size=tuple(wall_cfg["size"]),
                    pos=tuple(wall_cfg["pos"]),
                    fixed=True,
                    batch_fixed_verts=True,
                ),
                surface=gs.surfaces.Plastic(
                    color=tuple(wall_cfg["color"]),
                    roughness=wall_cfg.get("roughness", 0.8),
                ),
            )
        ...
        object_surface = gs.surfaces.Plastic(
            color=tuple(env_cfg.get("object_color", (1.0, 0.0, 0.0))),
            roughness=env_cfg.get("object_roughness", 0.65),
        )
        ...
        for vgeom in getattr(self.object, "vgeoms", []):
            if getattr(vgeom, "vmesh", None) is not None:
                vgeom.vmesh._surface = object_surface

        # == visualization camera (debug only, uses scene camera API unless HDR envmap is requested) ==
        if self.env_cfg.get("visualize_camera", False):
            envmap_hdr = self.env_cfg.get("envmap_hdr")
            use_nyx_camera = envmap_hdr or self.env_cfg.get("use_nyx_camera", False)
            if use_nyx_camera:
                env_maps = ()
                if envmap_hdr:
                    env_map = nps.EnvironmentMapAsset()
                    env_map.texture = envmap_hdr
                    env_map.layout = nps.EEnvMapLayout.LongLat
                    env_map.rotation = math.radians(self.env_cfg.get("envmap_rotation_deg", 30.0))
                    env_map.multiplier = self.env_cfg.get("envmap_multiplier", 1.5)
                    env_maps = (env_map,)
                light_fields = _make_nyx_light_fields(self.env_cfg.get("nyx_light_fields", ()))
                self.vis_cam = self.scene.add_sensor(
                    NyxCameraOptions(
                        res=tuple(self.env_cfg.get("envmap_resolution", (1280, 720))),
                        pos=tuple(self.env_cfg.get("envmap_camera_pos", (2.0, -1.5, 2.0))),
                        lookat=tuple(self.env_cfg.get("envmap_camera_lookat", (0.0, 0.0, 0.9))),
                        fov=self.env_cfg.get("envmap_camera_fov", 52),
                        up=tuple(self.env_cfg.get("envmap_camera_up", (0.0, 0.0, 1.0))),
                        near=self.env_cfg.get("envmap_camera_near", 0.02),
                        far=self.env_cfg.get("envmap_camera_far", 50.0),
                        spp=self.env_cfg.get("envmap_spp", 32),
                        render_mode=npr.ERenderMode.FastPathTracer,
                        lights=self.env_cfg.get("nyx_lights", []),
                        env_maps=env_maps,
...
ROBOT_ALUMINUM_LINK_NAMES = {"link7"}
ROBOT_VISUAL_MATERIALS = {
    "White": (1.0, 1.0, 1.0, 1.0),
    "Black": (0.0, 0.0, 0.0, 1.0),
    "Aluminum": (0.753, 0.753, 0.753, 1.0),
}


def _robot_material_name(link_name: str) -> str:
    if link_name in ROBOT_GRIPPER_LINK_NAMES:
        return "Black"
    if link_name in ROBOT_ALUMINUM_LINK_NAMES:
        return "Aluminum"
    return "White"


def _rewrite_robot_visual_urdf(robot_file: str, visual_materials=None) -> str:
    visual_materials = visual_materials or ROBOT_VISUAL_MATERIALS
    source_path = Path(robot_file).expanduser().resolve()
    tree = ET.parse(source_path)
    root = tree.getroot()

    for name, rgba in visual_materials.items():
        material = next((m for m in root.findall("material") if m.get("name") == name), None)
        if material is None:
            material = ET.SubElement(root, "material", {"name": name})
        color = material.find("color")
        if color is None:
            color = ET.SubElement(material, "color")
        color.set("rgba", " ".join(f"{v:g}" for v in rgba))
    ...
    for link in root.findall("link"):
        material_name = _robot_material_name(link.get("name", ""))
    ...
    target_path = Path(tempfile.gettempdir()) / f"{source_path.stem}_nyx_materials.urdf"
    tree.write(target_path, encoding="utf-8", xml_declaration=True)
    return str(target_path)


def _set_vgeom_surface(vgeom, surface, color):
    vgeom._surface = surface
    vgeom._color = color
    if getattr(vgeom, "vmesh", None) is not None:
        vgeom.vmesh._surface = surface
        vgeom.vmesh.set_color(color)


def apply_robot_visual_surfaces(robot_entity, visual_materials=None) -> None:
    visual_materials = visual_materials or ROBOT_VISUAL_MATERIALS
    surfaces = {
        "White": gs.surfaces.BSDF(color=visual_materials["White"][:3], roughness=0.28),
        "Black": gs.surfaces.BSDF(color=visual_materials["Black"][:3], roughness=0.35),
        "Aluminum": gs.surfaces.BSDF(color=visual_materials["Aluminum"][:3], metallic=1.0, roughness=0.22),
    }
    for vgeom in robot_entity.vgeoms:
        material_name = _robot_material_name(vgeom.link.name)
        _set_vgeom_surface(vgeom, surfaces[material_name], visual_materials[material_name])
...
class Manipulator:
    def __init__(self, num_envs: int, scene: gs.Scene, args: dict, device: str = "cpu"):
        ...
        robot_file = args.get("robot_file", "xml/franka_emika_panda/panda.xml")
        robot_visual_materials = args.get("robot_visual_materials", ROBOT_VISUAL_MATERIALS)
        if args.get("rewrite_robot_visual_urdf", True) and args.get("robot_morph", "mjcf") == "urdf":
            robot_file = _rewrite_robot_visual_urdf(robot_file, robot_visual_materials)
    ...
    def set_pd_gains(self):
        ...
        kp = self._args.get("dofs_kp", [4500, 4500, 3500, 3500, 2000, 2000, 2000, 100, 100])
        kv = self._args.get("dofs_kv", [450, 450, 350, 350, 200, 200, 200, 10, 10])
        force_lower = self._args.get("dofs_force_lower", [-87, -87, -87, -87, -12, -12, -12, -100, -100])
        force_upper = self._args.get("dofs_force_upper", [87, 87, 87, 87, 12, 12, 12, 100, 100])
        self._robot_entity.set_dofs_kp(torch.tensor(kp, device=self._device))
        self._robot_entity.set_dofs_kv(torch.tensor(kv, device=self._device))
        self._robot_entity.set_dofs_force_range(
            torch.tensor(force_lower, device=self._device),
            torch.tensor(force_upper, device=self._device),
        )

    def _init(self):
        self._arm_dof_dim = self._args.get("arm_dof_dim", self._robot_entity.n_dofs - 2)  # total number of arm joints
        self._gripper_dim = self._args.get("gripper_dof_dim", self._robot_entity.n_dofs - self._arm_dof_dim)  # number of gripper joints
    ...
    def go_to_goal(self, goal_pose: torch.Tensor, open_gripper: bool = True):
        ...
        q_pos = self._robot_entity.inverse_kinematics(
            ...
            quat=goal_pose[:, 3:7],
            init_qpos=init_qpos,
            max_samples=self._args.get("ik_max_samples", 50),
            max_solver_iters=self._args.get("ik_max_solver_iters", 20),
            damping=self._args.get("ik_damping", 0.01),
            dofs_idx_local=self._arm_dof_idx,
        )
        if open_gripper:
            q_pos[:, self._fingers_dof] = self._gripper_open_dof
